...
select * from n_tasks;
select t.*, m.status from n_task_messages m left join n_tasks t on t.id = m.task_id;
update n_tasks set task_finish_date = '2019-04-11 00:00:00' where n_tasks.id in (4,5);
update n_task_messages set status = 0;

update n_task_messages
set status = 1, n_tasks.status = 3
from n_tasks t
where t.id = task_id and t.task_finish_date < now();
